...
import { MaintenanceLandAwagSearchCustomService } from './service/maintenance-land-awagsearch-custom.service';
import { MaintenanceLandTransportdeptOnChangeCustomService } from './service/maintenance-land-transportdeptonchange-custom.service';
...
import { ROLE_BASED_VISIBILITY } from './maintenance-land-config';
import { LandService, Land } from './../../land.service';
 ...
 */
@Directive()
export class MaintenanceLandComponent extends AwagBaseComponent implements OnInit, AfterViewInit, AfterViewChecked, OnDestroy {

  /**
   * Providers definition for @Component annotation.
   */
  static readonly providers: any[] = [
    MaintenanceLandAwagSearchCustomService,
    MaintenanceLandTransportdeptOnChangeCustomService,
  ];
   ...
   * code list for 運輸局
   */
  transportdeptOptsCode: any[] = [];

  /**
   * ListController for list (社員一覧)
   */
  listListCtrl!: AwagListController;

  /**
   * selectedKey for list (社員一覧)
   */
   ...
   * @param {MaintenanceLandAwagSearchCustomService} awagSearchService
   * @param {MaintenanceLandTransportdeptOnChangeCustomService} transportdeptOnChangeService
   */
  constructor(
    ...
    protected awagSearchService: MaintenanceLandAwagSearchCustomService,
    protected transportdeptOnChangeService: MaintenanceLandTransportdeptOnChangeCustomService,
    protected landService : LandService,
  ) {
    super(router, awagCommon, awagContext, awagLogger, awagAlert, awagLanguageSupport);
    this.awagContext.setScreenKey('maintenance_land');
    if (this.awagContext.getModalContext() == null) {
      this.subscription.add(this.translate.get('awag.prj.uc.maintenance.sc.land.name').subscribe((res: string) => window.document.title = res));
    }
  ...
  ngOnInit() {
    this.awagOnInit();
    this.model = this.getModelOnInit('maintenance_land');
    ...
    this.landService.getLands().subscribe((data) => {
          ...
          // this.awagContext.setVModel('maintenance_land', this.model);
          this.awagContext.restoreSubContext(this.router.routerState.snapshot.root.queryParamMap.get('scid'));
          this.appOnInit();
        });
  ...
  protected appNameValueInit(refresh?: boolean) {
    // Name value
    this.awagLanguageSupport.getNameValueTranslated(this.appNameValue.transportdeptOpts, this.transportdeptOptsCode);
  }
  ...
  /**
   * @description
   * Event handling for awagSearch (初期処理)
   */
  awagSearch() {
    this.showLoading();
    this.awagContext.setVModel('maintenance_land', this.model);
  ...
  /**
   * @description
   * Pre API call logic for awagSearch (初期処理)
  ...
  /**
   * @description
   * API success logic for awagSearch (初期処理)
   * @param {*} res HTTP response
   */
  awagSearchSuccessLogic(res: any) {
    this.awagLogger.debug('MaintenanceLandComponent.awagSearchSuccessLogic() ');
  ...
  /**
   * @description
   * API success custom logic for awagSearch (初期処理)
   * @param {*} res HTTP response
  ...
  /**
   * @description
   * API error logic for  awagSearch (初期処理)
   * @param {*} error HTTP error response
   * @param {*} model Model data of screen
   */
  awagSearchErrorLogic(error: any, model: any) {
    this.awagLogger.debug('MaintenanceLandComponent.awagSearchErrorLogic() ');
  ...
  /**
   * @description
   * API error custom logic for awagSearch (初期処理)
   * @param {*} error HTTP error response
   * @param {*} model Model data of screen
   * @returns Edited model data
   */
  ...
  /**
   * @description
   * Event handling for cancel (戻る)
   */
  cancel() {
    this.awagContext.setVModel('maintenance_land', this.model);
    // screen transition to caller screen.
    const nextModel = this.cancelCustomLogic(this.model);
    const subContext = this.awagContext.getSubContext();
    if (subContext) {
      if (subContext.getAllowedMergeProperties()) {
        const baseModel = this.awagContext.getVModel(subContext.getBaseModelKey());
        this.mergeModel(baseModel, nextModel, subContext.getAllowedMergeProperties());
      }
      subContext.setNextModel(nextModel);
      subContext.setReturn(true);
      this.awagContext.setNextUrl(subContext.getReturnToPath());
    }
  ...
  /**
   * @description
   * Custom logic for cancel (戻る)
   * @param {*} model Model data of screen
   * @returns Edited model data
   */
  protected cancelCustomLogic(model: any) { return model; }
  ...
  select(index: number, item: any) {
    this.awagContext.setVModel('maintenance_land', this.model);
    // screen transition to caller screen.
    const nextModel = this.selectCustomLogic(this.model, index, item);
    const subContext = this.awagContext.getSubContext();
    if (subContext) {
      if (subContext.getAllowedMergeProperties()) {
        const baseModel = this.awagContext.getVModel(subContext.getBaseModelKey());
  ...
  /**
   * @description
   * Event handling for transportdeptOnChange (運輸局 onChange)
   * @param {number} index Index of selected row
   * @param {*} item Model data of selected item
   */
  transportdeptOnChange() {
    this.showLoading();
    this.awagContext.setVModel('maintenance_land', this.model);
    const param = this.transportdeptOnChangePreCallLogic(this.model);
    if (this.awagContext.isCancelApi()) {
      // cancel
      this.awagContext.setCancelApi(false);
      if (this.awagContext.getNextUrl() != null) {
      ...
      // main
      // this.subscription.add(this.transportdeptOnChangeService.execute(param).subscribe({
      //   next: (res) => this.transportdeptOnChangeSuccessLogic(res),
      //   error: (error) => this.transportdeptOnChangeErrorLogic(error, this.model)
      // }));
      // console.log(param);
      this.landService.getLandByDept(param.search.transportdept).subscribe((data) => {
          this.lands = data;
          this.model.list = this.lands;
          // console.log('this.model.list:',this.model.list);
          this.listListCtrl ??= new AwagListController(this.model.list, null);
  ...
  /**
   * @description
   * Pre API call logic for transportdeptOnChange (運輸局 onChange)
   * @param {*} model Model data of screen
   * @returns Edited model data
   */
  protected transportdeptOnChangePreCallLogic(model: any) { return model; }

  /**
   * @description
   * API success logic for transportdeptOnChange (運輸局 onChange)
   * @param {*} res HTTP response
   */
  transportdeptOnChangeSuccessLogic(res: any) {
    this.awagLogger.debug('MaintenanceLandComponent.transportdeptOnChangeSuccessLogic() ');
    this.awagLogger.debug(res);
    res ??= {};
    res = this.transportdeptOnChangeSuccessCustomLogic(res);
    this.model = res;
    if (res.awagMessage) {
      this.showMessage(res.awagMessage);
    }
  ...
  /**
   * @description
   * API success custom logic for transportdeptOnChange (運輸局 onChange)
   * @param {*} res HTTP response
   * @returns Edited HTTP response
   */
  protected transportdeptOnChangeSuccessCustomLogic(res: any) { return res; }

  /**
   * @description
   * API error logic for  transportdeptOnChange (運輸局 onChange)
   * @param {*} error HTTP error response
   * @param {*} model Model data of screen
   */
  transportdeptOnChangeErrorLogic(error: any, model: any) {
    this.awagLogger.debug('MaintenanceLandComponent.transportdeptOnChangeErrorLogic() ');
    this.awagLogger.warn(error);
    this.model = this.transportdeptOnChangeErrorCustomLogic(error, this.model);
    this.showError(error);
    this.hideLoading();
    this.awagAfterViewInit();
  }
  ...
  /**
   * @description
   * API error custom logic for transportdeptOnChange (運輸局 onChange)
   * @param {*} error HTTP error response
   * @param {*} model Model data of screen
   * @returns Edited model data
   */
  protected transportdeptOnChangeErrorCustomLogic(error: any, model: any) { return model; }

  selectedRowIndex: number | null = null;

  onRowClick(row: any, index: number): void {
    this.selectedRowIndex = index;
    this.select(index, row);
  }
}
